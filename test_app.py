...
app = Flask(__name__)
UPLOAD_FOLDER = os.path.abspath('static/uploads/')  # 使用绝对路径
PRESET_IMAGE_PATH = os.path.abspath('static/uploads/H2_clipped_heidi.tif')
...
def upload_file():
    ...
    # 生成唯一的文件名
    chla_uuid = str(uuid.uuid4())   # 生成chla的uuid
    tss_uuid = str(uuid.uuid4())    # 生成TSS的uuid

    # 保存Chl-a浓度分布图
    chla_output_path = os.path.join(app.config['UPLOAD_FOLDER'], f'chla_map_{chla_uuid}.png')
    tss_output_path = os.path.join(app.config['UPLOAD_FOLDER'], f'tss_map_{tss_uuid}.png')
    ...
    # 返回结果的链接
    return jsonify({
        "chla_map_url": f"/download?file=chla_map_{chla_uuid}.png",
        "tss_map_url": f"/download?file=tss_map_{tss_uuid}.png"
    })

@app.route('/download_chla', methods=['GET'])
def get_file_chla():
    image_path = PRESET_IMAGE_PATH
    ...
    # 生成Chl-a浓度分布图
    chla_map = generate_chla_map(chla_concentration, chla_output_path, nodata_value=-9999)
    tss_map = generate_tss_map(tss_concentration, tss_output_path, nodata_value=-9999)


    
    
    # 直接返回生成的文件
    return send_file(chla_output_path)  


@app.route('/download_TSS', methods=['GET'])
def get_file_tss():
    image_path = PRESET_IMAGE_PATH
    ...
    # 直接返回生成的文件
    return send_file(tss_output_path)   
